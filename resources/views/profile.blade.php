...
<head>
    ...
    <title>{{$user['username']}} | Connect</title>
                    ...
                    <div class="w-[20px] lg:w-[40px] lg:h-[40px]">
                        <button id="dropdownUserAvatarButton" data-dropdown-toggle="dropdownAvatar"
                            ...
                            <img class="w-[40px] h-[20px] lg:w-8 lg:h-8 rounded-full object-cover"
                                src="{{asset('uploads/user/profileImage/' . $currentuser['image']) ?? 'https://cdn.prod.website-files.com/62d84e447b4f9e7263d31e94/6399a4d27711a5ad2c9bf5cd_ben-sweet-2LowviVHZ-E-unsplash-1.jpeg'}}"
                                alt="user photo">
                        ...
                        <div id="dropdownAvatar"
                            ...
                            <div class="px-4 py-3 text-sm text-gray-900 dark:text-white">
                                <div><span>@</span>{{  $currentuser['username'] ?? 'User' }}</div>
                                <div class="font-medium truncate">{{ $currentuser['email'] ?? 'No Email' }}</div>
                            </div>
    ...
    <div class="container-width">
            ...
            <div class="flex lg:flex-row flex-col gap-10 lg:gap-20">
                ...
                <div class="flex flex-col lg:py-5">
                    <div class="flex items-center gap-2">
                        ...
                        @if ($currentuser['id'] === $user['id'])
                            <a href="{{route('editprofile')}}"
                                class="py-1 bg-gray-50 px-3 text-[15px] border-gray-200 border-[0.5px]">Edit
                                Profile</a>
                        @endif
                        ...
                        @else
                            @if ($currentuserData->following->contains($user->id))
                                <form action="{{ route('unfollow', $user->id) }}" method="POST">
                                    @csrf
                                    <button type="submit"
                                        class="py-1 float-end px-6 rounded-full hover:bg-gray-200 duration-100 text-[18px] border-gray-400 border-[0.5px]">Unfollow</button>
                                </form>
                            @else
                                <form action="{{ route('follow', $user->id) }}" method="POST">
                                    @csrf
                                    <button type="submit"
                                        class="py-1 float-end px-6 rounded-full hover:bg-gray-200 duration-100 text-[18px] border-gray-400 border-[0.5px]">Follow</button>
                                </form>
                            @endif
                        @endif
                    ...
                    <div class="flex justify-between items-center mt-4 gap-5">
                        <span class="text-[14px] font-bold">{{$user->posts->count()}} <span
                                class="font-normal">posts</span></span>
                        <span class="text-[14px] font-bold">{{$user->followers->count()}} <span
                                class="font-normal">followers</span></span>
                        <span class="text-[14px] font-bold">{{$user->following->count()}} <span
        ...
        @if ($userposts->isEmpty())
            <div class="flex items-center justify-center py-10">
                <h1 class="text-2xl font-bold text-gray-700">No Posts Yet</h1>
            </div>
        @else
                    ...
                    <div class="hover:opacity-90 cursor-pointer group" onclick="postModle({{$post->id}})">
                        <div>
                            <img class="object-cover w-full h-[100px] md:h-[180px] lg:h-[280px]"
                                ...
                                alt="">
                            <div class="hidden gap-2 group-hover:flex absolute ml-[7.3rem] -translate-y-40 text-white text-2xl">
                                <div class="flex items-center gap-2">
                                        ...
                                        <div class="flex flex-col ">
                                            ...
                                            <div class="mt-4">
                                                @foreach ($post->comments as $comment)
                                                    ...
                                                    <div class="text-[8px] text-gray-600 ml-2 mt-1">{{$comment->created_at->format('M d')}}</div>
                                                @endforeach
                                            </div>
                                        </div>
                                    </div>
